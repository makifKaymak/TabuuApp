//
//  GameQuestionSecondTeamBoxView.swift
//  TabuuApp
//
...
struct GameQuestionSecondTeamBoxView: View {
    @ObservedObject var viewModel: GameViewModel
    
    var body: some View {
        if viewModel.showEndViewPage {
            EndGameView(viewModel: viewModel)
        } else {
            VStack(spacing: 20) {
                HStack {
                    Text("Score: \(viewModel.teamBScore)")
                        .font(.headline)
                ...
                HStack {
                    ...
                    Button(action: {
                        viewModel.teamBScore -= 3 // Skor güncelle
                    ...
                    Button(action: {
                        viewModel.teamBScore += 1 // Skor güncelle
                        viewModel.loadNextQuestion() // Yeni soru al
                    }) {
                        Text("Correct")
                            .font(.headline)
            ...
            .onAppear {
                ...
                //viewModel.currentTeam = viewModel.btakimi
...
#Preview {
    GameQuestionSecondTeamBoxView(viewModel: GameViewModel())
}
